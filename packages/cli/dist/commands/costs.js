...
export function registerCostsCommand(program) {
    program
        .command('costs')
...
async function runCosts(opts, globals) {
    ...
    try {
        ...
        // Summary
        console.log(chalk.white('Total Cost:    ') + chalk.bold(`$${totalCost.toFixed(4)}`));
        console.log(chalk.white('Requests:      ') + rows.length.toLocaleString());
        console.log(chalk.white('Input Tokens:  ') + totalInput.toLocaleString());
        console.log(chalk.white('Output Tokens: ') + totalOutput.toLocaleString());
        // Budget bar
