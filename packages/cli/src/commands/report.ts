...
export function registerReportCommand(program: Command): void {
  program
    ...
    .action(async (opts: { failed: boolean }) => {
...
async function runReport(
  ...
  try {
    ...
    console.log(
      `  ${chalk.green('✓')} Ingested  ${String(data.fileStatus.ingested).padStart(5)}` +
    ...
    if (data.entityBreakdown.length === 0) {
      ...
      for (const row of data.entityBreakdown) {
        ...
        console.log(
          `  ${row.type.padEnd(14)} ${String(row.count).padStart(5)}  ${bar}  conf ${conf}%`,
        );
      }
    }
    ...
    if (totalContradictions === 0) {
      ...
      console.log(
        `  ${chalk.red('Active')}   ${data.contradictions.active}` +
        `   Resolved  ${data.contradictions.resolved}` +
        `   Dismissed ${data.contradictions.dismissed}`,
      );
      if (data.contradictions.active > 0) {
        console.log(
          chalk.dim(`  Severity: ${data.contradictions.highSeverity} high / ${data.contradictions.mediumSeverity} medium / ${data.contradictions.lowSeverity} low`),
        );
    ...
    if (totalInput === 0 && totalOutput === 0) {
      ...
      console.log(
        `  Input   ${totalInput.toLocaleString().padStart(10)} tokens\n` +
