...
export function registerFindCommand(program: Command): void {
  program
    ...
    .action(async (name: string, opts: { expand: string; type?: string }) => {
...
async function runFind(
  name: string,
  opts: { expand: string; type?: string },
...
function displayEntity(entity: Entity): void {
  console.log('');
...
function displayRelationship(
  ...
  console.log(
    `  ${direction} ${chalk.bold(rel.type)} ${chalk.cyan(targetName)} ${chalk.dim(targetType)}` +
