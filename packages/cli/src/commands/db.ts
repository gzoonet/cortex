...
export function registerDbCommand(program: Command): void {
  ...
  dbCmd
    ...
    .action(async (sourcePath: string, opts: { force?: boolean }) => {
...
async function runDbClean(sourcePath: string, force: boolean, globals: GlobalOptions): Promise<void> {
  ...
  try {
    const ok = await confirm(
      `Hard-delete all entities, relationships, and file records under "${sourcePath}"?`,
      force,
    );
    if (!ok) {
      console.log('Aborted.');
...
async function runDbReset(force: boolean, globals: GlobalOptions): Promise<void> {
  const config = loadConfig({ configDir: globals.config ? resolve(globals.config) : undefined });
  const store = new SQLiteStore({ dbPath: config.graph.dbPath, backupOnStartup: false });
  ...
  try {
    ...
    const ok = await confirm(
      `Reset database? This will delete ${stats.entityCount} entities, ${stats.relationshipCount} relationships, and ${stats.fileCount} file records. Projects will be kept.`,
      force,
    );
    if (!ok) {
      console.log('Aborted.');
...
async function runDbPrune(force: boolean, globals: GlobalOptions): Promise<void> {
  const config = loadConfig({ configDir: globals.config ? resolve(globals.config) : undefined });
  const store = new SQLiteStore({ dbPath: config.graph.dbPath, backupOnStartup: false });
