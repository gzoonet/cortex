...
async function runModelsList(globals: GlobalOptions): Promise<void> {
...
async function runModelsPull(model: string, globals: GlobalOptions): Promise<void> {
  if (!globals.quiet) {
...
async function runModelsTest(globals: GlobalOptions): Promise<void> {
  const config = loadConfig({ configDir: globals.config ? resolve(globals.config) : undefined });
  const router = new Router({ config });
  const local = router.getLocalProvider();
...
async function runModelsInfo(globals: GlobalOptions): Promise<void> {
  const config = loadConfig({ configDir: globals.config ? resolve(globals.config) : undefined });
  const router = new Router({ config });
  const local = router.getLocalProvider();
