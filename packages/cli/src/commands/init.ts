...
export function registerInitCommand(program: Command): void {
  program
    ...
    .action(async (opts: { mode?: string; nonInteractive: boolean }) => {
...
type CloudPresetKey = keyof typeof CLOUD_PRESETS;

const VRAM_HINT =
  process.platform === 'darwin'  ? '(Apple menu → About This Mac → More Info → Graphics)' :
  process.platform === 'linux'   ? '(run: nvidia-smi or rocm-smi)' :
                                   '(Task Manager → Performance → GPU → Dedicated GPU Memory)';

async function runInit(
  opts: { mode?: string; nonInteractive: boolean },
  globals: GlobalOptions,
): Promise<void> {
  const config = getDefaultConfig();

  if (opts.nonInteractive) {
    if (opts.mode) config.llm.mode = opts.mode as typeof config.llm.mode;
    writeConfig(config, globals);
    return;
  }

  if (!globals.quiet) console.log(chalk.bold('\nCortex Setup\n'));

  // ── Step 1: What do you have? (derives mode automatically) ────────────────
  const { hasOllama } = await inquirer.prompt<{ hasOllama: boolean }>([{
    type: 'confirm',
    name: 'hasOllama',
  ...
  const { hasApiKey } = await inquirer.prompt<{ hasApiKey: boolean }>([{
    type: 'confirm',
    name: 'hasApiKey',
  ...
  let mode: typeof config.llm.mode;
  ...
  if (usesLocal) {
    console.log('');
    const { vramGb } = await inquirer.prompt<{ vramGb: number }>([{
      type: 'number',
      name: 'vramGb',
      message: `How many GB of VRAM does your GPU have? ${VRAM_HINT}`,
      default: 8,
      validate: (v: number) => (v > 0 && Number.isFinite(v)) || 'Enter a number greater than 0',
    }]);

    const gpuTier: keyof typeof GPU_PRESETS =
      vramGb >= 24 ? 'high' : vramGb >= 8 ? 'mid' : 'low';
    ...
    if (useManual) {
      const answers = await inquirer.prompt<{ model: string; numCtx: number }>([
        {
          type: 'input',
          name: 'model',
  ...
  let selectedPresetKey: CloudPresetKey = 'gemini';
  if (usesCloud) {
    console.log('');
    ...
    selectedPresetKey = cloudProvider as CloudPresetKey;
  ...
  console.log('');
  const { watchDirs } = await inquirer.prompt<{ watchDirs: string }>([{
    type: 'input',
    name: 'watchDirs',
    message: 'Directories to watch (comma-separated):',
    default: '.',
  }]);
  config.ingest.watchDirs = watchDirs.split(',').map((d: string) => d.trim());

  // ── Step 5: Monthly budget (if cloud involved) ─────────────────────────────
  if (usesCloud) {
    const { budget } = await inquirer.prompt<{ budget: number }>([{
      type: 'number',
      name: 'budget',
...
function writeConfig(config: Record<string, unknown> | ReturnType<typeof getDefaultConfig>, globals: GlobalOptions): void {
  ...
  const configPath = resolve(process.cwd(), 'cortex.config.json');
